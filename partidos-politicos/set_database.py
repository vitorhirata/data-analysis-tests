...
def create_candidates_elected():

    print("Creating table candidates_elected")
    # Create table name_gender
    ...
    # If the table already exist drop it
    try:
        cur.execute('''DROP TABLE candidates_elected; ''')
    except:
        pass

    cur.execute('''CREATE TABLE candidates_elected
        (UF varchar,
        CARGO varchar,
        NUMERO_DO_CANDIDATO varchar,
        NOME_DO_CANDIDATO varchar,
        NOME_DO_CANDIDATO_URNA varchar,
        PARTIDO varchar,
        COLIGACAO varchar,
        TURNO varchar,
        VOTOS_NOMINAIS varchar); ''')

    file = open('data/candidatos_eleitos.csv', 'r', encoding = 'ISO8859-14')
    file2 = open('data/candidatos_eleitos2.csv', 'r', encoding = 'ISO8859-14')
    cur.copy_expert('''COPY candidates_elected FROM STDIN DELIMITER ';' CSV HEADER ENCODING 'ISO8859-14'; ''', file)
    cur.copy_expert('''COPY candidates_elected FROM STDIN DELIMITER ';' CSV HEADER ENCODING 'ISO8859-14'; ''', file2)

    cur.execute('''
        ALTER TABLE candidates_elected
        DROP COLUMN NOME_DO_CANDIDATO_URNA,
        DROP COLUMN COLIGACAO,
        DROP COLUMN TURNO,
        DROP COLUMN VOTOS_NOMINAIS;''')
    print("Finish candidates_elected creation.")
